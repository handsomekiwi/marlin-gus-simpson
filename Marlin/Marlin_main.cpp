...
// Extruder offsets
  ...
  float home_height = HOMED_HEIGHT;
  float delta_radius =(SIZE/2)/SIN_60;
  float delta_tower1_x = -SIZE/2.0; // front left tower
  float delta_tower1_y = -COS_60*delta_radius;
  float shoulder_height = SHOULDER_Z-HUB_Z;
...
inline void gcode_M206() {
  ...
  inline void gcode_M665() {
   ...
   recalc_delta_settings(delta_radius, delta_diagonal_rod);*/
    if (code_seen('H')) home_height = code_value();
  }
  /**
   * M666: Set delta endstop adjustment
   */
...
void clamp_to_software_endstops(float target[3]) {
  ...
  delta[TOWER_1] = sqrt(sq(cartesian[X_AXIS]-delta_tower1_x)
      ...
      +sq(cartesian[Z_AXIS]-shoulder_height));
 ...
 /*delta[TOWER_1] = sqrt(delta_diagonal_rod_2_tower_1
   ...
   void adjust_delta(float cartesian[3]) {
      ...
      delta[Y_AXIS] += offset;
      delta[Z_AXIS] += offset;
     /*delta[TOWER_1] =  300.0-sqrt(sq(cartesian[X_AXIS]+offset-delta_tower1_x)
      +sq(cartesian[Y_AXIS]+offset-delta_tower1_y)
      +sq(cartesian[Z_AXIS]+offset-shoulder_height));
      delta[TOWER_2] =  300.0-sqrt(sq(cartesian[X_AXIS]+offset-delta_tower2_x)
      +sq(cartesian[Y_AXIS]+offset-delta_tower2_y)
      +sq(cartesian[Z_AXIS]+offset-shoulder_height));
    delta[TOWER_3] = 300.0-sqrt(sq(cartesian[X_AXIS]+offset-delta_tower3_x)
      +sq(cartesian[Y_AXIS]+offset-delta_tower3_y)
      +sq(cartesian[Z_AXIS]+offset-shoulder_height));*/
